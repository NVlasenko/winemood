...
export const ProfileFavorites = ({
  ...
  const { favoriteWines, hasLoadedFavorites, isFavorite, toggleFavorite } =
    useFavorites();

  const { isOpen, isVisible, titleRef, toggleOpen } = useExpandableSection();

  const displayedFavorites = hasLoadedFavorites
    ? favoriteWines
    : initialFavoriteWines;

  const initialFavorites = useMemo(
    () => displayedFavorites.slice(0, INITIAL_VISIBLE_COUNT),
    [displayedFavorites]
  );

  const extraFavorites = useMemo(
    () => displayedFavorites.slice(INITIAL_VISIBLE_COUNT),
    [displayedFavorites]
  );
          ...
          <div className="profile-favorites__grid">
            {initialFavorites.map((wine, index) => (
              <WineCatalogCard
                ...
                wine={wine}
                index={index}
                isFavorite={hasLoadedFavorites ? isFavorite(wine.id) : true}
