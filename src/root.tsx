...
import { checkBackendAvailability } from "@/shared/api/assets/siteAssetsApi";
...
const getMoodThemeFromCookie = (cookieHeader: string | null): MoodTheme => {
  if (!cookieHeader) {
    return "default";
  }
  ...
  for (const cookie of cookies) {
    ...
    if (key !== MOOD_THEME_COOKIE_KEY) {
      continue;
    }

    const value = decodeURIComponent(valueParts.join("="));

    if (value && value in moodThemeValues) {
...
const getThemeStyle = (moodTheme: MoodTheme): React.CSSProperties => {
...
export async function loader({ request }: { request: Request }) {
  ...
  const moodTheme = getMoodThemeFromCookie(cookieHeader);

  const authToken = await accessTokenCookie.parse(cookieHeader);
  ...
  if (authToken) {
    try {
      favoriteWines = await userApi.getFavorites({
        authToken,
      });

      hasLoadedFavorites = true;
    } catch (error) {
      console.error("Root SSR favorites load failed:", error);
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const moodTheme = data?.moodTheme ?? "default";

  return (
    <html lang="en" style={getThemeStyle(moodTheme)}>
      <head>
        <meta charSet="UTF-8" />
...
export function ErrorBoundary() {
  ...
  const moodTheme = rootData?.moodTheme ?? "default";
  ...
  useEffect(() => {
    ...
    const checkBackend = async () => {
      try {
        ...
        if (!cancelled) {
          retryTimer = window.setTimeout(checkBackend, 5_000);
        }
      }
    };
  ...
  if (isBackendUnavailable) {
    return (
      <div style={getThemeStyle(moodTheme)}>
        <AppLoadingProvider>
          <NavigationLoadingOverlay forceVisible />
        </AppLoadingProvider>
      </div>
    );
  }
...
export default function Root() {
  const { moodTheme, favoriteWines, hasLoadedFavorites } =
    useLoaderData<typeof loader>();

  return (
    <QueryClientProvider client={queryClient}>
      <MoodThemeProvider initialMoodTheme={moodTheme}>
        <AuthProvider>
          <AuthRequiredProvider>
            <FavoritesProvider
              initialFavoriteWines={favoriteWines}
              initialHasLoadedFavorites={hasLoadedFavorites}
            >
              <QuizSessionProvider>
                <AppLoadingProvider>
