...
type MoodThemeProviderProps = React.PropsWithChildren<{
...
const applyMoodTheme = (moodTheme: MoodTheme) => {
...
const saveMoodTheme = (moodTheme: MoodTheme) => {
  localStorage.setItem(MOOD_THEME_STORAGE_KEY, moodTheme);

  document.cookie = `${MOOD_THEME_COOKIE_KEY}=${encodeURIComponent(
    moodTheme
...
export const MoodThemeProvider = ({
  ...
  const [moodTheme, setMoodThemeState] = useState<MoodTheme>(initialMoodTheme);

  const setMoodTheme = useCallback((theme: MoodTheme) => {
